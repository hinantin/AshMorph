...
compile:
	...
	@rm -f verb.composition.prq.foma | perl extractEntries.pl --file nroot.prq.foma --outputfilename "verb.composition.prq.foma" --flag "@VCMP" --title "NOUN INCORPORATION (Verb-noun composition)" --header "V=S=noun" --separator 1 --replace1 1 --replace3 1 --source3 "NRoot" --target3 "IncNRoot"
	@rm -f verb.composition.len.prq.foma | perl extractEntries.pl --file nroot.prq.foma --outputfilename "verb.composition.len.prq.foma" --flag "@VCMP" --title "NOUN INCORPORATION (Verb-noun composition)" --header "V=S=LEN=noun" --separator 1 --replace3 1 --source3 "NRoot" --target3 "IncNRoot"
	...
	@rm -f vroot.redup.foma | perl extractEntries.pl --file vroot.prq.foma --outputfilename "vroot.redup.foma" --flag "@REDUP" --title "VERBAL REDUPLICATION" --header "VRootREDUPin" --replace2 1 --source2 "!###" --target2 ""
